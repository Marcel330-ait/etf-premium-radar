(function () {
  ...
  async function query(code) {
    ...
    return { code, name:String(q.f58), type:isEtf(code) ? "etf" : "stock", price, change:Number(q.f170 || 0) / 100, pe:Number(q.f162) > 0 ? Number(q.f162) / 100 : null, nav:estimatedNav, premium:estimatedNav ? (price / estimatedNav - 1) * 100 : null, position, low, high, time:(nav && nav.gztime) || new Date().toLocaleString("zh-CN", {hour12:false}) };
  }
  ...
  function render() {
    ...
    $("updateTime").textContent=(zh?"行情更新时间：":"Quote time: ")+q.time; renderBudget();
  }
  ...
  function renderBudget(){ const q=state.quote; if(!q)return; const zh=state.lang==="zh"; const lot=q.price*100; const shares=Math.floor(state.budget/lot)*100; const spent=shares*q.price; $("budgetPill").textContent=money(state.budget); $("lotCost").textContent=money(lot); $("maxShares").textContent=shares.toLocaleString()+ (zh?" 份 / 股":" units / shares"); $("maxSpent").textContent="≈ "+money(spent); $("cashLeft").textContent=money(Math.max(0,state.budget-spent));
    ...
    $("trancheList").innerHTML=steps.map((s,i)=>{const part=state.budget*s.r; const anchor=q.type==="etf"?(q.nav||q.price):q.price; const target=anchor*(1+s.m/100); const units=Math.floor(part/(target*100))*100; return `<article><span class="step">0${i+1}</span><div><h3>${zh?s.zh:s.en}</h3><p>${zh?s.nzh:s.nen}</p></div><div><small>${zh?"参考价格不高于":"Reference price at or below"}</small><b>¥${fmt(target,q.type==="etf"?3:2)}</b></div><div><small>${zh?"本笔预算 / 约":"Budget / approx."}</small><b>${money(part)} · ${units} ${zh?"份 / 股":"units"}</b></div></article>`;}).join("");
  }
